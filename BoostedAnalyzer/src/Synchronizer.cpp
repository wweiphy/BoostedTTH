...
{
    out << "run,lumi,event,is_e,is_mu,is_ee,is_emu,is_mumu,n_jets,n_btags,lep1_pt,lep1_eta,lep1_iso,lep1_pdgId,lep1_idSF,lep1_recoSF,lep1_isoSF,lep1_seed,lep2_pt,lep2_eta,lep2_iso,lep2_pdgId,lep2_idSF,lep2_isoSF,lep2_seed,jet1_pt,jet1_eta,jet1_phi,jet1_jesSF,jet1_jesSF_up,jet1_jesSF_down,jet1_jesSF_PileUpDataMC_down,jet1_jesSF_RelativeFSR_up,jet1_jerSF_nominal,jet1_deepjet,jet1_PUJetId,jet1_PUJetDiscriminant,jet1_seed,jet2_pt,jet2_eta,jet2_phi,jet2_jesSF,jet2_jesSF_up,jet2_jesSF_down,jet2_jesSF_PileUpDataMC_down,jet2_jesSF_RelativeFSR_up,jet2_jerSF_nominal,jet2_deepjet,jet2_PUJetId,jet2_PUJetDiscriminant,jet2_seed,MET_pt,MET_phi,mll,ttHFCategory,ttHFGenFilterTag,n_interactions,puWeight,csvSF,csvSF_lf_up,csvSF_hf_down,csvSF_cErr1_down,pdf_up,pdf_down,pdf_Weight_1,muRWeight_up,muFWeight_down,isr_up,isr_down,fsr_up,fsr_down,triggerSF,triggerSF_up,top_pt_weight,bdt_output,dnn_ttH_output,dnn_ttbb_output";
    if (addExtendedInfo) {
        out << ",jet3_pt,jet3_eta,jet3_deepjet,jet4_pt,jet4_eta,jet4_deepjet,jet5_pt,jet5_eta,jet5_deepjet,jet6_pt,jet6_eta,jet6_deepjet,jet7_pt,jet7_eta,jet7_deepjet";
...
//    pointerToMVAvars->SetWP(CSVHelper::GetWP(input.era, CSVHelper::CSVwp::Medium, "DeepJet"));
    ...
    for (std::vector<pat::Muon>::const_iterator iMuon = input.selectedMuonsLoose.begin(); iMuon != input.selectedMuonsLoose.end(); ++iMuon) {
        ...
        if (iMuon->pt() > lep1_pt) {
            ...
            lep1_pt = iMuon->pt();
            lep1_eta = iMuon->eta();
            if (iMuon->hasUserFloat("relIso"))
                lep1_iso = iMuon->userFloat("relIso");
            if (iMuon->hasUserFloat("ReconstructionSF"))
                lep1_recoSF = iMuon->userFloat("ReconstructionSF");
            lep1_pdgId = iMuon->pdgId();
            if (iMuon->hasUserInt("deterministicSeed"))
                lep1_seed = (uint32_t)iMuon->userInt("deterministicSeed");
            ...
            lep2_eta = iMuon->eta();
            if (iMuon->hasUserFloat("relIso"))
                lep2_iso = iMuon->userFloat("relIso");
            lep2_pdgId = iMuon->pdgId();
            if (iMuon->hasUserInt("deterministicSeed"))
                lep2_seed = (uint32_t)iMuon->userInt("deterministicSeed");
    ...
    for (std::vector<pat::Electron>::const_iterator iEle = input.selectedElectronsLoose.begin(); iEle != input.selectedElectronsLoose.end(); ++iEle) {
        ...
        if (iEle->pt() > lep1_pt) {
            ...
            lep1_pt = iEle->pt();
            lep1_eta = iEle->eta();
            if (iEle->hasUserFloat("relIso"))
                lep1_iso = iEle->userFloat("relIso");
            lep1_pdgId = iEle->pdgId();
            if (iEle->hasUserInt("deterministicSeed"))
                lep1_seed = (uint32_t)iEle->userInt("deterministicSeed");
            ...
            if (iEle->hasUserFloat("ReconstructionSF"))
                lep1_recoSF = iEle->userFloat("ReconstructionSF");
        } else if (iEle->pt() > lep2_pt) {
            lep2_pt = iEle->pt();
            lep2_eta = iEle->eta();
            if (iEle->hasUserFloat("relIso"))
                lep2_iso = iEle->userFloat("relIso");
            lep2_pdgId = iEle->pdgId();
            if (iEle->hasUserInt("deterministicSeed"))
                lep2_seed = (uint32_t)iEle->userInt("deterministicSeed");
    ...
    for (uint i = 0; i < selectionsSL.size(); i++) {
        if (!selectionsSL[i]->IsSelected(input, cutflowSL)) {
            is_SL = false;
            if (i == 0)
                pass_FilterSelection = 0;
            if (i == 1)
                pass_VertexSelection = 0;
            if (i == 2)
                pass_LeptonSelection = 0;
            if (i == 3)
                pass_METSelection = 0;
            ...
            if (i == 0)
                pass_FilterSelection = 1;
            if (i == 1)
                pass_VertexSelection = 1;
            if (i == 2)
                pass_LeptonSelection = 1;
            if (i == 3)
                pass_METSelection = 1;
        }
    }
    if (is_SL) {
        if (abs(lep1_pdgId) == 11) {
            is_e = 1;
            is_mu = 0;
            triggerSF = leptonsfs["ElectronTriggerSF"];
            triggerSF_up = leptonsfs["ElectronTriggerSF_Up"];
        } else if (abs(lep1_pdgId) == 13) {
            is_mu = 1;
            is_e = 0;
            triggerSF = leptonsfs["MuonTriggerSF"];
            triggerSF_up = leptonsfs["MuonTriggerSF_Up"];
        }
    } else {
        is_e = 0;
        is_mu = 0;
...
//        jetcsvs.push_back(CSVHelper::GetJetCSV(*j, "DeepJet"));
    ...
    //     cout << "N_Electrons: " << input.selectedElectrons.size() << endl;
    //     cout << "N_Muons: " << input.selectedMuons.size() << endl;
    //     cout << "Lepton: "
    //          << "pt: " << lepvecs[0].Pt() << " eta: " << lepvecs[0].Eta() << " phi: " << lepvecs[0].Phi() << " e: " << lepvecs[0].E() << endl;
    ...
    for (uint i = 0; i < selectionsDL.size(); i++) {
        if (!selectionsDL[i]->IsSelected(input, cutflowDL)) {
            is_DL = false;
            if (i == 2)
                pass_DiLeptonSelection = 0;
            break;
        } else {
            if (i == 2)
                pass_DiLeptonSelection = 1;
        }
    }
    if (is_DL) {
...
//        jet1_csv = CSVHelper::GetJetCSV(input.selectedJets.at(0), "DeepJet");
        ...
        if (input.selectedJets.at(0).hasUserInt("pileupJetId:fullId"))
            jet1_PUJetId = input.selectedJets.at(0).userInt("pileupJetId:fullId");
        if (input.selectedJets.at(0).hasUserFloat("pileupJetId:fullDiscriminant")) {
            jet1_PUJetDiscriminant = input.selectedJets.at(0).userFloat("pileupJetId:fullDiscriminant");
        }
        if (input.selectedJets.at(0).hasUserFloat("HelperJES"))
            jet1_jesSF = input.selectedJets.at(0).userFloat("HelperJES");
        if (input.selectedJets.at(0).hasUserFloat("HelperJESup"))
            jet1_jesSF_up = input.selectedJets.at(0).userFloat("HelperJESup");
        if (input.selectedJets.at(0).hasUserFloat("HelperJESdown"))
            jet1_jesSF_down = input.selectedJets.at(0).userFloat("HelperJESdown");
        if (input.selectedJets.at(0).hasUserFloat("HelperJER"))
            jet1_jerSF_nominal = input.selectedJets.at(0).userFloat("HelperJER");
        ...
        if (input.selectedJets.at(0).hasUserFloat("HelperJESPileUpDataMCdown"))
            jet1_jesSF_PileUpDataMC_down = input.selectedJets.at(0).userFloat("HelperJESPileUpDataMCdown");
        if (input.selectedJets.at(0).hasUserFloat("HelperJESPileUpDataMCdown"))
            cout << input.selectedJets.at(0).userFloat("HelperJESPileUpDataMCdown") << endl;
        if (input.selectedJets.at(0).hasUserFloat("HelperJESRelativeFSRup"))
            jet1_jesSF_RelativeFSR_up = input.selectedJets.at(0).userFloat("HelperJESRelativeFSRup");
    }
...
//        jet2_csv = CSVHelper::GetJetCSV(input.selectedJets.at(1), "DeepJet");
        ...
        if (input.selectedJets.at(1).hasUserInt("pileupJetId:fullId"))
            jet2_PUJetId = input.selectedJets.at(1).userInt("pileupJetId:fullId");
        if (input.selectedJets.at(1).hasUserFloat("pileupJetId:fullDiscriminant")) {
            jet2_PUJetDiscriminant = input.selectedJets.at(1).userFloat("pileupJetId:fullDiscriminant");
        }
        if (input.selectedJets.at(1).hasUserFloat("HelperJES"))
            jet2_jesSF = input.selectedJets.at(1).userFloat("HelperJES");
        if (input.selectedJets.at(1).hasUserFloat("HelperJESup"))
            jet2_jesSF_up = input.selectedJets.at(1).userFloat("HelperJESup");
        if (input.selectedJets.at(1).hasUserFloat("HelperJESdown"))
            jet2_jesSF_down = input.selectedJets.at(1).userFloat("HelperJESdown");
        if (input.selectedJets.at(1).hasUserFloat("HelperJER"))
            jet2_jerSF_nominal = input.selectedJets.at(1).userFloat("HelperJER");
        ...
        if (input.selectedJets.at(1).hasUserFloat("HelperJESPileUpDataMCdown"))
            jet2_jesSF_PileUpDataMC_down = input.selectedJets.at(1).userFloat("HelperJESPileUpDataMCdown");
        if (input.selectedJets.at(1).hasUserFloat("HelperJESRelativeFSRup"))
            jet2_jesSF_RelativeFSR_up = input.selectedJets.at(1).userFloat("HelperJESRelativeFSRup");
    }
...
//        jet7_csv = CSVHelper::GetJetCSV(input.selectedJetsLoose.at(6), "DeepJet");
    ...
    trig_el = input.triggerInfo.IsTriggered("HLT_Ele35_WPTight_Gsf_v*") || input.triggerInfo.IsTriggered("HLT_Ele28_eta2p1_WPTight_Gsf_HT150_v*")
        || input.triggerInfo.IsTriggered("HLT_Ele32_WPTight_Gsf_L1DoubleEG_v*"); // 2017 trigger...
...
//            if (CSVHelper::PassesCSV(*jet, "DeepJet", CSVHelper::CSVwp::Medium, input.era))
    ...
    if (input.weights.count("Weight_PU") > 0)
    ...
    if (input.weights.count("Weight_CSV") > 0)
        csvSF = input.weights.at("Weight_CSV");
    if (input.weights.count("Weight_CSVLFup") > 0)
        csvSF_lf_up = input.weights.at("Weight_CSV") * input.weights.at("Weight_CSVLFup");
    if (input.weights.count("Weight_CSVHFdown") > 0)
        csvSF_hf_down = input.weights.at("Weight_CSV") * input.weights.at("Weight_CSVHFdown");
    if (input.weights.count("Weight_CSVCErr1down") > 0)
        csvSF_cErr1_down = input.weights.at("Weight_CSV") * input.weights.at("Weight_CSVCErr1down");

    if (input.weights.count("Weight_scale_variation_muR_2p0_muF_1p0") > 0)
        muR_up = input.weights.at("Weight_scale_variation_muR_2p0_muF_1p0");
    if (input.weights.count("Weight_scale_variation_muR_1p0_muF_0p5") > 0)
        muF_down = input.weights.at("Weight_scale_variation_muR_1p0_muF_0p5");
    if (input.weights.count("Weight_TopPt") > 0)
        top_pt_weight = input.weights.at("Weight_TopPt");
    if (input.weights.count("Weight_LHA_306000_up") > 0)
        pdf_up = input.weights.at("Weight_LHA_306000_up");
    if (input.weights.count("Weight_LHA_306000_down") > 0)
        pdf_down = input.weights.at("Weight_LHA_306000_down");
    if (input.weights.count("Weight_pdf_variation_306001") > 0)
        pdf_Weight_1 = input.weights.at("Weight_pdf_variation_306001");
    if (input.weights.count("GenWeight_isr_Def_up") > 0)
        isr_up = input.weights.at("GenWeight_isr_Def_up");
    if (input.weights.count("GenWeight_fsr_Def_up") > 0)
        fsr_up = input.weights.at("GenWeight_fsr_Def_up");
    if (input.weights.count("GenWeight_isr_Def_down") > 0)
        isr_down = input.weights.at("GenWeight_isr_Def_down");
    if (input.weights.count("GenWeight_fsr_Def_down") > 0)
        fsr_down = input.weights.at("GenWeight_fsr_Def_down");
    ...
    //         lepcharges.push_back(input.selectedElectron1010671772sLoose[i].charge());
    //     }1010671772
    //     for(int i=0;i<int(input.selectedMuonsLoose.size1010671772());i++) {
    //         lepvecs.push_back(BoostedUtils::GetTLorentz1010671772Vector(input.selectedMuonsLoose[i].p4()));
    //         lepcharges.push_back(input.selectedMuonsLoo1010671772se[i].charge());
    //     }
    ...
    //     //dnn_ttbb_output = dnn_output.ttbb();
    //     //dnn_ttH_output = dnn_output.ttH();1010671772
    // }

    bool print = false;
    if (dataset == "NA" && (is_SL || is_DL))
        print = true;
    if (dataset == "el" && is_e)
        print = true;
    if (dataset == "mu" && is_mu)
        print = true;
    if ((dataset == "elel" || dataset == "el") && is_ee)
        print = true;
    if ((dataset == "elmu" || dataset == "el" || dataset == "mu") && is_emu)
        print = true;
    if ((dataset == "mumu" || dataset == "mu") && is_mumu)
        print = true;
    ...
    // cout << "PRINT it!!! " << pdf_Weight_1 << endl;
    if (print) {
        out << boost::format("%i,%i,%i,\
%i,%i,%i,%i,%i,\
%i,%i,\
%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%i,\
%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%i,\
...
%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f")
                ...
                % n_jets % n_btags
                % lep1_pt % lep1_eta % lep1_iso % lep1_pdgId % lep1_idSF % lep1_recoSF % lep1_isoSF % lep1_seed
                ...
                % pdf_up % pdf_down % pdf_Weight_1
                % muR_up % muF_down % isr_up % isr_down % fsr_up % fsr_down % triggerSF % triggerSF_up % top_pt_weight % bdt_output % dnn_ttH_output % dnn_ttbb_output;
        if (addExtendedInfo) {
